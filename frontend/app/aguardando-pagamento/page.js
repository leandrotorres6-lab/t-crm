...
export default function AguardandoPagamentoPage() {
  ...
  // Atualização em tempo real ao criar pagamento de qualquer tela
  useEffect(() => {
    const handler = (e) => {
      const { lead, paymentDueDate, id } = e.detail || {}
      if (!paymentDueDate) return
      setAll(prev => {
        const exists = prev.find(l => String(l.id) === String(id))
        const entry  = lead ? { ...lead, paymentDueDate } : { id, paymentDueDate }
        if (exists) return prev.map(l => String(l.id) === String(id) ? { ...l, paymentDueDate } : l)
        return [entry, ...prev]
      })
    }
    window.addEventListener('tcrm:payment-created', handler)
    return () => window.removeEventListener('tcrm:payment-created', handler)
  }, [])
