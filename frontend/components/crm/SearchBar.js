...
export default function SearchBar({ onResults, onClear, autoFocus = false }) {
  ...
  // Refs estáveis — não causam re-render e não criam loops
  ...
  // Mantém latestFilters sempre atualizado sem ser dependência de callbacks
  ...
  // Única função de busca — estável, lê query/filters de refs (sem dependências que causam loop)
  const executeSearch = useCallback(async () => {
    ...
    try {
      ...
      if (e?.name !== 'AbortError') onClear?.()
    } finally {
      setSearching(false)
    }
  }, [currentAgent, onResults, onClear])  // deps estáveis — não mudam a cada render
  ...
  const handleInput = useCallback((e) => {
    ...
    if (v === latestQuery.current) return  // sem mudança real — não agenda
    latestQuery.current = v
    setQuery(v)
    if (!v.trim() && !latestFilters.current.column && !latestFilters.current.assignee && !latestFilters.current.product) {
      clearTimeout(debounceRef.current)
      onClear?.()
      return
    }
    scheduleSearch()
  }, [scheduleSearch, onClear])

  // Filtros — agenda nova busca mas não recria handleInput
  ...
  const clear = useCallback(() => {
    ...
    latestFilters.current = { column: '', assignee: '', product: '' }
    onClear?.()
  }, [onClear])
        ...
        <div className="absolute top-full left-0 right-0 mt-2 p-3 rounded-xl shadow-2xl z-30"
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
            <div>
              ...
              <select value={filters.column} onChange={e => updateFilter('column', e.target.value)}
          ...
          {(filters.column || filters.assignee || filters.product) && (
            <button onClick={() => { setFilters({ column: '', assignee: '', product: '' }); latestFilters.current = { column: '', assignee: '', product: '' }; scheduleSearch() }}
